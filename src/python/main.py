...
def main() -> None:
    ...
    rpc = RpcClient(cfg.rpc_endpoint, cfg.request_delay)
    db = Database(cfg.dsn)
    ...
    if args.check:
        try:
            ver = rpc.get_client_version()
            chain = rpc.get_chain_id()
            tip = rpc.get_latest_block_number()
    ...
    if start is None:
        ...
        if cp is not None:
            ...
            start = rpc.get_latest_block_number()
